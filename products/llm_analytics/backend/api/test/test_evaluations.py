...
from products.llm_analytics.backend.models.evaluations import EvaluationConfig
...
class TestEvaluationConfigsApi(APIBaseTest):
    def test_unauthenticated_user_cannot_access_evaluation_configs(self):
        self.client.logout()
        response = self.client.get(f"/api/environments/{self.team.id}/evaluation_configs/")
        self.assertEqual(response.status_code, status.HTTP_401_UNAUTHORIZED)

    def test_can_create_evaluation_config(self):
        response = self.client.post(
            f"/api/environments/{self.team.id}/evaluation_configs/",
            {
        ...
        self.assertEqual(EvaluationConfig.objects.count(), 1)

        evaluation_config = EvaluationConfig.objects.first()
        ...
        self.assertEqual(evaluation_config.metadata, {"key": "value"})
    ...
    def test_can_retrieve_list_of_evaluation_configs(self):
        EvaluationConfig.objects.create(name="Evaluation 1", prompt="Prompt 1", team=self.team, created_by=self.user)
        EvaluationConfig.objects.create(name="Evaluation 2", prompt="Prompt 2", team=self.team, created_by=self.user)
    ...
    def test_can_get_single_evaluation_config(self):
        evaluation_config = EvaluationConfig.objects.create(
            name="Test Evaluation",
            description="Test Description",
            enabled=True,
            prompt="Test prompt",
        ...
        response = self.client.get(f"/api/environments/{self.team.id}/evaluation_configs/{evaluation_config.id}/")
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_can_edit_evaluation_config(self):
        ...
        response = self.client.patch(
            f"/api/environments/{self.team.id}/evaluation_configs/{evaluation_config.id}/",
            {
                "name": "Updated Name",
    ...
    def test_delete_method_returns_405(self):
        ...
        response = self.client.delete(f"/api/environments/{self.team.id}/evaluation_configs/{evaluation_config.id}/")
    ...
    def test_can_search_evaluation_configs(self):
        ...
        # Search by name
        response = self.client.get(f"/api/environments/{self.team.id}/evaluation_configs/?search=accuracy")
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(len(response.data["results"]), 1)
        self.assertEqual(response.data["results"][0]["name"], "Accuracy Evaluation")

        # Search by description
        response = self.client.get(f"/api/environments/{self.team.id}/evaluation_configs/?search=performance")
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(len(response.data["results"]), 1)
    ...
    def test_can_filter_by_enabled_status(self):
        ...
        response = self.client.get(f"/api/environments/{self.team.id}/evaluation_configs/?enabled=true")
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(len(response.data["results"]), 1)
        self.assertEqual(response.data["results"][0]["name"], "Enabled Evaluation")

        # Filter for disabled only
        response = self.client.get(f"/api/environments/{self.team.id}/evaluation_configs/?enabled=false")
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(len(response.data["results"]), 1)
    ...
    def test_cannot_access_other_teams_evaluation_configs(self):
        ...
        other_evaluation = EvaluationConfig.objects.create(
        ...
        response = self.client.get(f"/api/environments/{self.team.id}/evaluation_configs/{other_evaluation.id}/")
