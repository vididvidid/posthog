...
from .max_tools import CreateFeatureFlagTool, FeatureFlagLoopNode, FeatureFlagToolkit
...
class TestCreateFeatureFlagTool(BaseTest):
    ...
    async def test_arun_impl_no_key_provided(self, mock_create_flag):
        """Test key generation when no key is provided (None)"""
        tool = self._setup_tool()

        # Mock LLM response with no key (None) - simulating new optional behavior
        mock_output = FeatureFlagCreationSchema(
            key=None,  # No key provided - should be generated from name
...
class TestFeatureFlagLoopNode(BaseTest):
    def setUp(self):
        super().setUp()

    def _setup_node(self):
        """Helper to create a FeatureFlagLoopNode instance"""
        return FeatureFlagLoopNode(team=self.team, user=self.user, toolkit_class=FeatureFlagToolkit)
    ...
    async def test_get_existing_feature_flags_summary_empty(self):
        ...
        summary = await node._get_existing_feature_flags_summary()
