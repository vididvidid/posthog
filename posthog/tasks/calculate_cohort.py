...
from posthog.models.cohort.util import (
    get_all_dependency_cohorts,
    get_all_dependent_cohorts,
...
def increment_version_and_enqueue_calculate_cohort(cohort: Cohort, *, initiating_user: Optional[User]) -> None:
    dependent_cohorts = get_all_dependent_cohorts(cohort)
    dependency_cohorts = get_all_dependency_cohorts(cohort)
    related_cohorts = dependent_cohorts + dependency_cohorts
    if related_cohorts:
        logger.info("cohort_has_dependencies", cohort_id=cohort.id, dependent_count=len(related_cohorts))
